...
export default function App() {
  const [data, add_row] = useState([{id: "id", name: 'Name', expirationDate: 'Days Remaining', 
  ...
  const addNewItem = () => {
    ...
    newState.push({id: test, name: food, expirationDate: tempDay, showTrash:true, servings: tempServing});
    let newState2 = [...newState].sort((a, b) => a.expirationDate - b.expirationDate);
    let newState3 = []
    for (var i = 0; i < newState2.length; i++) {
      newState3.push({id: i, name: newState2[i].name, expirationDate: newState2[i].expirationDate,
  ...
  const deleteItem = (id) => {
    ...
    for (var i = 0; i < newState.length; i++) {
      newState2.push({id: i, name: newState[i].name, expirationDate: newState[i].expirationDate,
  ...
  //           <Text style={styles.cell_text}>{item.expirationDate}</Text>
  //       </View>
  ...
  // )
   ...
   return (
    <Menu currentStorage = {currentStorage}> </Menu>
